...
- DB file location resolves via `src/lib/db/paths.js` (`DATA_DIR`, else `~/.sembilan-router/`).
- Usage/logs (`src/lib/usageDb.js`, `usage.json` + `log.txt`) still live under `~/.sembilan-router` and do **not** follow `DATA_DIR`.
