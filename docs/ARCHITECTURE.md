# Sembilan Router Architecture

_Last updated: 2026-02-06_

## Executive Summary

Sembilan Router is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[Sembilan Router Local Process]
...
- main state: `${DATA_DIR}/db.json` (or `~/.sembilan-router/db.json`)
- usage stats: `~/.sembilan-router/usage.json`
- request log lines: `~/.sembilan-router/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[Sembilan Router Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.sembilan-router` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/sembilan-router && npm run build`
- Build Docker image: `cd /root/dev/sembilan-router && docker build -t sembilan-router .`
