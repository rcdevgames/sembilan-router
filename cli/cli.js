...
// Subcommands (`sembilan-router xai video …`) run against an already-running gateway
...
// Self-heal SQLite runtime deps (sql.js + better-sqlite3) into ~/.sembilan-router/runtime
...
// Identifiers for killAllAppProcesses - only kill sembilan-router specifically
const PROCESS_IDENTIFIERS = [
  'sembilan-router'  // Only package name - avoid killing other apps
];
...
function getAppDataDir() {
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || "", "sembilan-router")
    : path.join(os.homedir(), ".sembilan-router");
}
...
// Kill all sembilan-router processes
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: real node process running sembilan-router/cli.js, or next-server.
            // Avoids killing editors/grep/strace/cursor that just have "sembilan-router" in cmdline.
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("sembilan-router") && (cmd.includes("cli.js") || cmd.includes("\\sembilan-router") || cmd.includes("/sembilan-router")))
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: real node process running sembilan-router/cli.js, or next-server.
            // Avoids killing grep/strace/editors/cursor that incidentally match "sembilan-router".
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("sembilan-router") && (cmd.includes("cli.js") || cmd.includes("/sembilan-router")))
              || cmd.includes("next-server");
            if (isAppProcess) {
              const parts = line.trim().split(/\s+/);
              const pid = parts[1];
...
function startServer(updatePromise) {
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 Sembilan Router is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 Sembilan Router is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
  ...
  function tryRestart(code) {
    ...
    if (restartCount >= MAX_RESTARTS) {
      ...
      try {
        const dbPath = path.join(os.homedir(), process.platform === "win32" ? path.join("AppData", "Roaming", "sembilan-router", "db.json") : path.join(".sembilan-router", "db.json"));
