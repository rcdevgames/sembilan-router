...
const APP_NAME = "sembilan-router";
const APP_LABEL = "com.sembilan-router.autostart";
 ...
 *   2. `process.argv[1]` if it's our cli.js — true when sembilan-router is currently
 ...
 * SIGTERM to the running process. When the running sembilan-router cli.js was itself
...
<dict>
    ...
    <string>/tmp/sembilan-router.log</string>
    <key>StandardErrorPath</key>
    <string>/tmp/sembilan-router.error.log</string>
</dict>
</plist>`;
...
function enableWindows(cliPath) {
  ...
  // `sembilan-router.cmd` lookup that depended on the npm prefix path.
...
Comment=Sembilan Router API Proxy
