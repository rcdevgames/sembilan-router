...
async function buildCodexHeader() {
  ...
  const { installed, hasSembilan Router, config } = result.data;
  if (!installed) return `Status:   ${COLORS.red}✗ Codex CLI not installed${COLORS.reset}`;

  if (!hasSembilan Router) {
    return [
...
async function buildDroidHeader() {
  ...
  const { installed, hasSembilan Router, settings } = result.data;
  ...
  // Extract Sembilan Router custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:Sembilan Router-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract Sembilan Router provider config
  const provider = settings?.models?.providers?.["sembilan-router"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("sembilan-router/") ? primary.replace("sembilan-router/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasSembilan Router, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use Sembilan Router\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
