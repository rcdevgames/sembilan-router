/**
 * `sembilan-router xai video` — generate a Grok Imagine video through the local
 * sembilan-router gateway and save the result as an MP4 file.
 *
...
const HELP = `
Usage: sembilan-router xai video --prompt "..." [options]

Generate a Grok Imagine video via your local sembilan-router gateway
...
Options:
  ...
  --api-key <key>         sembilan-router API key (or env NINE_ROUTER_API_KEY)
...
async function run(argv) {
  ...
  try {
    ...
    const connectionId = create.headers["x-sembilan-router-connection-id"] || null;
