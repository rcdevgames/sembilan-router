...
export default function ProfilePage() {
  ...
  const [usernameForm, setUsernameForm] = useState({ current: "", new: "" });
  ...
  const handleUsernameChange = async (e) => {
    e.preventDefault();
    const newUsername = usernameForm.new.trim();
    if (!newUsername) {
    ...
    try {
      const res = await fetch("/api/settings", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          currentPassword: usernameForm.current,
          newUsername,
        }),
      });
      ...
      setUsernameStatus({ type: "error", message: "An error occurred" });
    } finally {
  ...
  const handleExportDatabase = async (password) => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `sembilan-router-backup-${stamp}.json`;
          ...
          <div className="flex flex-col gap-3 pt-4 border-t border-border">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 rounded-lg bg-bg border border-border gap-2">
              <div>
                ...
                <p className="text-xs sm:text-sm text-text-muted font-mono break-all">~/.sembilan-router/db/data.sqlite</p>
              </div>
            </div>
            ...
            <div className="flex items-start sm:items-center justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs sm:text-sm text-text-muted">
                  When ON, dashboard requires username + password. When OFF, access without login.
                </p>
              </div>
              <Toggle
            ...
            {settings.requireLogin === true && (
              <div className="flex flex-col gap-6 pt-4 border-t border-border/50">
                ...
                <form onSubmit={handleUsernameChange} className="flex flex-col gap-4">
                  <div>
                    <p className="font-medium text-sm sm:text-base">Change Username</p>
                    <p className="text-xs sm:text-sm text-text-muted">
                      Enter your current password and a new username. The new username must be different.
                    </p>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="flex flex-col gap-2">
                      <label className="text-xs sm:text-sm font-medium">New Username</label>
                      <Input
                        type="text"
                  ...
                  {usernameStatus.message && (
                    <p className={`text-xs sm:text-sm ${usernameStatus.type === "error" ? "text-red-500" : "text-green-500"}`}>
                      {usernameStatus.message}
                    </p>
                  )}
                  <div className="pt-1">
                    <Button type="submit" variant="primary" loading={usernameLoading} className="w-full sm:w-auto">
                      Update Username
                    </Button>
                  </div>
                </form>
