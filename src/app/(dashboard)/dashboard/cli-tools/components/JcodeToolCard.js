...
export default function JcodeToolCard({
  ...
  useEffect(() => {
    if (jcodeStatus?.installed && !hasInitializedModel.current) {
      ...
      const provider = jcodeStatus.config?.providers?.["sembilan-router"];
      if (provider) {
  ...
  const getManualConfigs = () => {
    ...
    const configToml = `[providers.sembilan-router]
type = "openai-compatible"
base_url = "${getEffectiveBaseUrl()}"
auth = "bearer"
api_key_env = "JCODE_9ROUTER_API_KEY"
env_file = "provider-sembilan-router.env"
default_model = "${selectedModel || "cc/claude-opus-4-7"}"
requires_api_key = true

[[providers.sembilan-router.models]]
id = "${selectedModel || "cc/claude-opus-4-7"}"`;
    ...
    return [
      ...
      {
        filename: "~/.config/jcode/provider-sembilan-router.env",
        content: envContent,
      },
    ];
  };
          ...
          {!checkingJcode && jcodeStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {jcodeStatus?.config?.providers?.["sembilan-router"]?.base_url && (
                ...
                <div className="flex flex-col gap-1 p-3 bg-blue-500/5 border border-blue-500/20 rounded-lg">
                  ...
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile sembilan-router</code>
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile sembilan-router --model {selectedModel || "cc/claude-opus-4-7"}</code>
                </div>
              </div>
