...
// POST - Apply Sembilan Router config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing Sembilan Router entry or append
    const idx = config.findIndex((e) => e.name === "Sembilan Router");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove Sembilan Router entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "Sembilan Router");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "Sembilan Router removed from Copilot config",
    });
  } catch (error) {
