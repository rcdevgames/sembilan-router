...
// Check if settings has Sembilan Router config
const hasSembilan RouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["sembilan-router"];
};

// Read per-agent models.json and return current model id (without "sembilan-router/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["sembilan-router"]?.models;
...
const writeAgentModels = async (agentDir, model, baseUrl, apiKey) => {
  ...
  existing.providers["sembilan-router"] = {
    baseUrl,
...
// POST - Update Sembilan Router settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `sembilan-router/${model}`;

    // Remove all old sembilan-router/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("sembilan-router/"))
    ...
    // Add fresh sembilan-router models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`sembilan-router/${m}`] = {};
    });

    // Remove old sembilan-router model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("sembilan-router/")) {
    ...
    // Update models.providers.sembilan-router with all models
    settings.models.providers["sembilan-router"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `sembilan-router/${agentModel}` };
        return agent;
      });
...
export async function DELETE() {
  try {
    ...
    // Remove Sembilan Router from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["sembilan-router"];
    ...
    // Remove sembilan-router models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("sembilan-router/"));
    ...
    // Reset agents.defaults.model.primary if it uses sembilan-router
    if (settings.agents?.defaults?.model?.primary?.startsWith("sembilan-router/")) {
