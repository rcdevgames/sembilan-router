...
const hasSembilan RouterConfig = (auth) => {
  ...
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("sembilan-router");
};
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "Sembilan Router", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "Sembilan Router settings removed from Kilo Code" });
  } catch (error) {
