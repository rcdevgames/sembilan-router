...
const hasSembilanRouterConfig = (config) => {
  ...
  return !!config.provider["sembilan-router"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["sembilan-router"];
    ...
    return NextResponse.json({
      ...
      configPath: getConfigPath(),
        opencode: {
          ...
          activeModel: config?.model?.startsWith("sembilan-router/") ? config.model.replace(/^sembilan-router\//, "") : null,
...
// POST - Apply Sembilan Router as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_sembilan-router";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing sembilan-router provider entry and its models
    const existingProvider = config.provider["sembilan-router"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["sembilan-router"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `sembilan-router/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `sembilan-router/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("sembilan-router/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove Sembilan Router provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["sembilan-router"]?.models) {
      delete config.provider["sembilan-router"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["sembilan-router"].models).length === 0) {
        delete config.provider["sembilan-router"];
        if (config.model?.startsWith("sembilan-router/")) delete config.model;
      } else if (config.model === `sembilan-router/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["sembilan-router"].models);
        config.model = `sembilan-router/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire sembilan-router provider
      if (config.provider) delete config.provider["sembilan-router"];
      if (config.model?.startsWith("sembilan-router/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("sembilan-router/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "Sembilan Router settings removed from OpenCode",
    });
  } catch (error) {
