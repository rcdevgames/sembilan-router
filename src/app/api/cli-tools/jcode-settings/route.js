...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-sembilan-router.env");
};
...
const hasSembilan RouterConfig = (config) => {
  ...
  if (providers["sembilan-router"]) return true;
...
export async function POST(request) {
  try {
    ...
    config.providers["sembilan-router"] = {
    ...
    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile sembilan-router",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["sembilan-router"];
    ...
    return NextResponse.json({
      success: true,
      message: "sembilan-router configuration removed from jcode",
    });
  } catch (error) {
