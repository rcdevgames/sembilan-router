...
// Collect PIDs of all sembilan-router-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from sembilan-router install dir or wrapper cli.js
        const isAppProcess = lower.includes("sembilan-router") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "sembilan-router");
  }
  return path.join(os.homedir(), ".sembilan-router");
}
...
// Resolve npx/sembilan-router binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx sembilan-router` — works regardless of global bin path changes after npm i -g
